...
% ----------------------------------------------------------------------- %
function [h,interp_x, interp_y, interp_z] = plot_topography(ch_list, values, make_contour, system, ...
    ...
    plot(head_x, head_y, 'Color', 'k', 'LineWidth',2);
    hold on;
    ...
    plot(nose_x, nose_y, 'Color', 'k', 'LineWidth',2);
    hold on;
    ...
    plot(ear_x_right+head_rho+offset, ear_y_right, 'Color', 'k', 'LineWidth',2); hold on;
    plot(ear_x_left-head_rho-offset, ear_y_left, 'Color', 'k', 'LineWidth',2); hold on;

    % Plotting the electrodes
    % [ch_x, ch_y] = pol2cart((pi/180).*(locations.theta(idx)+90), locations.radius(idx));
    if plot_channels, he = scatter(ch_x, ch_y, 20,'k','LineWidth',0.75); end
